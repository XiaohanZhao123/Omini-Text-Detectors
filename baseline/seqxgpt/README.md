...
## Quick Start: AI Text Boundary Detection

SeqXGPT detects AI-generated text at the **word/sentence level**, returning character-level intervals for AI-written content.

### Basic Usage

```python
import sys
sys.path.insert(0, 'baseline/seqxgpt')
from local_infer import SeqXGPT

# Initialize with 4 models (best accuracy ~90%)
detector = SeqXGPT(
    checkpoint_path='zcahjl3/seqxgpt-detector',  # Auto-downloads from HuggingFace
    classifier_type='transformer',
    feature_models=['gpt2', 'gpt-neo-1.3b', 'gpt-j-6b', 'llama-7b'],
    device='cuda:0',
    feature_devices=['cuda:0', 'cuda:0', 'cuda:1', 'cuda:2']  # Distribute across GPUs
)

# Detect AI content
text = "This is human-written text. AI systems can generate fluent text that mimics human writing."
result = detector.predict(text)

print(f"Overall label: {result['pred_label']}")  # 'human', 'ai', or 'mixed'
print(f"AI intervals: {result['ai_intervals']}")  # [[start, end], ...] character positions
print(f"Word predictions: {result['predictions']}")  # BIOES labels per word
...
```python
{
    ...
    'predictions': list,      # BIOES labels per word (e.g., 'B-gpt2', 'S-human')
    'ai_intervals': list,     # Character-level AI spans [[start, end], ...]
    'pred_label': str,        # Overall: 'human', 'ai', or 'mixed'
    'word_positions': list    # (start, end) for each word
}
```

### Using with Omni-text Pipeline
...
print(result['metadata']['ai_intervals'])  # AI-written character spans
print(result['metadata']['pred_label'])    # human/ai/mixed
```
...
|-------|---------------|------|
| GPT-2 | `gpt2` | ~0.5GB |
| GPT-Neo 1.3B | `EleutherAI/gpt-neo-1.3B` | ~2.5GB |
