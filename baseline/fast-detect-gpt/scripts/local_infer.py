...
class FastDetectGPT:
    def __init__(self, args):
        ...
        if args.sampling_model_name != args.scoring_model_name:
            ...
            tokens_scoring = self.scoring_tokenizer(test_text, return_tensors="pt").input_ids
            tokens_sampling = self.sampling_tokenizer(test_text, return_tensors="pt").input_ids
            if not torch.equal(tokens_scoring, tokens_sampling):
                raise ValueError(
                    f"\n❌ Error: Incompatible tokenizers!\n"
                    f"   Sampling model: {args.sampling_model_name}\n"
                    f"   Scoring model: {args.scoring_model_name}\n\n"
                    f"These models use different tokenizers and cannot be used together.\n"
                    f"Compatible combinations:\n"
                    f"  • gpt-j-6B + gpt-neo-2.7B (both use GPT-2 tokenizer)\n"
                    f"  • gpt-neo-2.7B + gpt-neo-2.7B (same tokenizer)\n"
                    f"  • falcon-7b + falcon-7b-instruct (both use Falcon tokenizer)\n"
                    f"  • Use same model for both: --sampling_model_name {args.scoring_model_name}\n"
                )

        # Determine actual device for tensor operations
        # When using device_map, use the first device from the model's device map
        if hasattr(self.scoring_model, 'hf_device_map'):
            devices = list(self.scoring_model.hf_device_map.values())
            self.tensor_device = devices[0] if devices else 'cuda:0'
        elif args.device == 'auto':
            self.tensor_device = 'cuda:0'
        elif ',' in args.device:
            # Multi-GPU: use first GPU
            self.tensor_device = f'cuda:{args.device.split(",")[0]}'
        elif args.device.isdigit():
            self.tensor_device = f'cuda:{args.device}'
        else:
            self.tensor_device = args.device
        ...
        # Use pre-computed parameters if available, otherwise use default
        if key in distrib_params:
            self.classifier = distrib_params[key]
        else:
            # Default parameters (averaged from available combinations)
            self.classifier = {'mu0': 0.0, 'sigma0': 1.0, 'mu1': 2.3, 'sigma1': 1.9}
            print(f'\n⚠️  Warning: Model combination "{key}" not pre-calibrated.')
            print(f'Using default distribution parameters. Results may be less accurate.')
            print(f'Recommended combinations: {list(distrib_params.keys())}\n')

    # compute conditional probability curvature
    def compute_crit(self, text):
        tokenized = self.scoring_tokenizer(text, truncation=True, return_tensors="pt", padding=True, return_token_type_ids=False).to(self.tensor_device)
        ...
        with torch.no_grad():
            ...
            if self.args.sampling_model_name == self.args.scoring_model_name:
                ...
                tokenized = self.sampling_tokenizer(text, truncation=True, return_tensors="pt", padding=True, return_token_type_ids=False).to(self.tensor_device)
                if not torch.all(tokenized.input_ids[:, 1:] == labels):
                    raise RuntimeError(
                        "Tokenizer mismatch detected during inference. "
                        "This should not happen as tokenizers were verified at initialization."
                    )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--device', type=str, default="auto",
                        help='Device: "auto" (all GPUs), "0" (cuda:0), "0,1,2" (specific GPUs), "cpu"')
