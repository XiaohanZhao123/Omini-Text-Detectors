...
from baseline.glimpse.scripts.data_builder import load_data, save_data
from baseline.glimpse.scripts.metrics import (
    get_precision_recall_metrics,
    get_roc_metrics,
)
from baseline.glimpse.scripts.probability_distributions import (
...
class OpenAIGPT:
    ...
    def create_client_openai(self):
        ...
        return OpenAI(base_url=self.args.api_base, api_key=self.args.api_key)
    ...
    def eval(self, text):
        ...
        while nretry > 0:
            try:
                ...
                kwargs = {
                    "model": self.args.scoring_model_name,
                    "max_tokens": 0,
                    "echo": True,
                    "logprobs": self.max_topk,
                }
...
class PdeFastDetectGPT(PdeBase):
    def __call__(self, args, item):
        ...
        discrepancy = (log_likelihood.sum(axis=-1) - mean_ref.sum(axis=-1)) / np.sqrt(
            var_ref.sum(axis=-1)
        )
...
def evaluate_passages(args, gpt):
    ...
    results = []
    for idx in tqdm.tqdm(range(n_samples), desc="Evaluating passages"):
        ...
        try:
            ...
            results.append({"original": original_result, "sampled": sampled_result})
        except Exception as ex:
            print(ex)
...
def experiment(args):
    ...
    # evaluate criterion
    estimators = {
        "geometric": GeometricDistribution(args.top_k, args.rank_size),
    ...
    for idx in tqdm.tqdm(
        range(n_samples), desc=f"Computing criteria PDE with {estimator} distribution"
    ):
        ...
        original_crit = dict(
            [
                (name, criterion_fns[name](args, original_result))
        ...
        sampled_crit = dict(
            [
                (name, criterion_fns[name](args, sampled_result))
                for name in criterion_fns
            ]
        )
        # result
    ...
    for name in criterion_fns:
        ...
        predictions = {
            "real": [
                ...
                if x["original_crit"] is not None
            ],
            "samples": [
                x["sampled_crit"]
                for x in results[name]
                if x["sampled_crit"] is not None
            ],
        }
        print(
            f"Total {len(predictions['real'])}, Real mean/std: {np.mean(predictions['real']):.2f}/{np.std(predictions['real']):.2f}, Samples mean/std: {np.mean(predictions['samples']):.2f}/{np.std(predictions['samples']):.2f}"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output_file",
        type=str,
        default="./exp_test/results/xsum_gpt-3.5-turbo.babbage-002",
    )
    parser.add_argument("--dataset", type=str, default="xsum")
    parser.add_argument(
        "--dataset_file", type=str, default="./exp_test/data/xsum_gpt-3.5-turbo"
    )
    parser.add_argument("--scoring_model_name", type=str, default="davinci-002")
    parser.add_argument(
        "--api_base", type=str, default="https://xxxx.openai.azure.com/"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="geometric",
        choices=["geometric", "zipfian", "mlp"],
    )
    parser.add_argument(
        "--prompt",
        type=str,
        default="prompt3",
        choices=["prompt0", "prompt1", "prompt2", "prompt3", "prompt4"],
    )
