...
from baseline.glimpse.scripts.data_builder import load_data
from baseline.glimpse.scripts.model import load_model, load_tokenizer
...
class MlpDistributionModel(nn.Module):
    ...
    def forward(self, input, top_k):
        ...
        p_rest = 1 - p_top
        probs = torch.cat(
            [input[:, :top_k], p_rest * torch.softmax(logits[:, top_k:], dim=-1)],
            dim=-1,
        )
        return probs
    ...
    def predict(self, x, top_k):
        x = (
            x[: self.max_topk]
            if len(x) >= self.max_topk
            else x + [1] * (self.max_topk - len(x))
        )
        x = np.array([x])
...
def get_distrib_data(args):
    ...
    for dataset in datasets:
        ...
        for source in sources:
            ...
            for idx in tqdm.tqdm(range(n_samples), desc="Get distributions"):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model_file", type=str, default="./exp_test/model/mlp_distribution_model.pt"
    )
