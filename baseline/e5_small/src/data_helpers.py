...
__all__ = [
    "process_data",
    "is_processed_data",
    "train_test_split_unequal_class",
    "train_test_split_equal_class",
    "label_counter",
    "check_column_type",
    "check_column_values",
    "check_column_string",
...
def process_data(data, text_col="generation", label_col="model", reduced=False):
    ...
    def ensure_dataset(df):
        ...
        # Check if the object is a pandas DataFrame
            ...
            raise TypeError(
                "Input must be a pandas DataFrame or a datasets.Dataset object."
            )
    ...
    if is_processed_data(data):
        ...
        if reduced:
            data = data.remove_columns(
                [col for col in data.column_names if col not in ["text", "labels"]]
            )
    ...
    if label_col != "labels":
        ...
        def mutate_column(temp):
            if label_col != "labels":
                temp["labels"] = temp[
                    "model"
                ]  # Create 'labels' column with 'model' values
            return temp
    ...
    # Drop NAs
    data = data.filter(
        lambda x: x["text"] is not None and x["labels"] is not None, desc="Dropping NAs"
    )

    # Transform `labels` to binary 0/1 if needed
    def transform_labels(input):
        if (
            input["labels"] == 0 or input["labels"] == 1
        ):  # check if the label is already 0 or 1
            return {"labels": input["labels"]}
        elif (
            "human" in str(input["labels"]).lower()
        ):  # check if the label contains "human" in a case-insensitive manner
            return {"labels": 0}
    ...
    if not check_column_values(data, "labels"):
        data = data.map(
            lambda x: {"labels": int(x["labels"])}, desc="Converting labels to numeric"
        )
...
def train_test_split_unequal_class(data, test_size=0.2, seed=42):
    ...
    if isinstance(split_data, dict) and "train" in split_data and "test" in split_data:
        ...
        raise ValueError(
            "Unexpected format for split data. Expected a dictionary with keys 'train' and 'test'."
        )
