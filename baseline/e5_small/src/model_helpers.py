...
__all__ = [
    ...
    "custom_collate_fn",
    "inference_model",
    "save_inference_to_csv",
    "read_inference_as_DataFrame",
    "read_inference_as_Dataset",
]
...
def inference_model(model, data, nthreads=4):
    ...
    if isinstance(data, ProcessedData):
        ...
        raise TypeError(
            "Input must be a `ProcessedData` instance or a list of strings or a pandas DataFrame."
        )
    ...
    dataloader = DataLoader(
        data, batch_size=1, collate_fn=custom_collate_fn, shuffle=False
    )
    ...
    predicted_ds = Dataset.from_dict(
        {"Predicted_Probs(1)": [prob for prob in predictions]}
    )
...
def save_inference_to_csv(prediction_results, data, csv_file_path):
    ...
    if len(predictions) != len(data):
        raise ValueError(
            "Length of predictions must match the length of original data."
        )
    ...
    combined_data = pd.concat(
        [data_df.reset_index(drop=True), predictions_df.reset_index(drop=True)], axis=1
    )
