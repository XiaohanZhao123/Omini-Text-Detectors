...
Note: Binoculars requires GPU (~14GB VRAM for both models).
"""
...
def example_1_quick_detection():
    ...
    print(f"Binoculars Score: {result['metadata']['binoculars_score']:.6f}")
...
def example_2_batch_processing():
    ...
    pipe = pipeline("ai-text-detection", model="binoculars", mode="accuracy")
    ...
    for i, result in enumerate(results, 1):
        ...
        print(
            ...
            f"Binoculars: {result['metadata']['binoculars_score']:.4f}"
        )
    print()
...
def example_3_config_driven():
    ...
    print(f"Configuration loaded from: omini_text/configs/binoculars.yaml")
    print(
        f"Label: {result['label']} "
        f"({'AI-generated' if result['label'] == 1 else 'Human-written'})"
    )
...
def example_4_mode_comparison():
    ...
    for mode in modes:
        print(f"\nMode: {mode}")
        pipe = pipeline("ai-text-detection", model="binoculars", mode=mode)
        result = pipe(text)
        print(f"  Threshold: {result['metadata']['threshold']:.6f}")
        print(f"  Binoculars Score: {result['metadata']['binoculars_score']:.6f}")
        print(f"  Prediction: {result['metadata']['prediction']}")
...
def example_5_understanding_scores():
    """Example 5: Understanding Binoculars scores."""
    print("=" * 80)
    print("Example 5: Understanding Binoculars Scores")
    print("=" * 80)
    ...
    The score is the ratio: perplexity / cross-perplexity
    - AI text: Similar perplexity across models → ratio closer to 1 → lower score
    - Human text: Different perplexity → ratio varies more → higher score
    """)
...
def example_6_error_handling():
    ...
    try:
        ...
        print(f"  Score: {result['score']:.4f}")
        print("  ⚠️  Warning: Very short texts may give unreliable results")
    ...
    try:
        pipe = pipeline("ai-text-detection", model="binoculars")
        result = pipe("")
        print(f"  Unexpected success with empty text")
    except Exception as e:
...
if __name__ == "__main__":
    print("\n")
    print("╔" + "=" * 78 + "╗")
    print("║" + " " * 20 + "Binoculars Detector Usage Examples" + " " * 23 + "║")
    print("╚" + "=" * 78 + "╝")
    print()

    print("⚠️  IMPORTANT: Binoculars requires GPU (~14GB VRAM for both models).")
    ...
    try:
        ...
        if not torch.cuda.is_available():
            ...
            print("Binoculars may be very slow or fail on CPU-only systems.")
            ...
            if gpu_mem < 14:
                print(f"   ⚠️  Warning: Recommended VRAM is 14GB+")
    ...
    try:
        ...
        print(
            "4. The binoculars package is installed (pip install -e baseline/binoculars)"
        )
        import traceback
